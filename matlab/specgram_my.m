function [yo,fo,to] = specgram_my(varargin)
...
%
%   Мой вариант с линейной шкалой, типичный вызов:
%   specgram_my(y,4096,96000,[],2048)

% Пример вызова: specgram_my(y,2048,Fs,[],0);

error(nargchk(1,5,nargin,'struct'))
[msg,x,nfft,Fs,window,noverlap]=specgramchk(varargin);
if ~isempty(msg), error(generatemsgid('SigErr'),msg); end
    
nx      = length(x); %Длина входного вектора
nwind   = length(window); %Длина взвешивающего окна
...
if nargout == 0
    newplot;
    if length(t)==1
        imagesc([0 1/f(2)],f,(abs(y)+eps));axis xy; colormap(jet)
    else
        ...
        %imagesc(t,f,(log10(abs(y)+eps)));axis xy; colormap(jet)
        
        %% ===== Здесь настраиваем вывод =================================
        Scale = 1;%/50; % Подбирается вручную
        %our_idxs = find(f > 2e4);
        our_idxs = find(f > 0); %0.5e4);
        imagesc(t,f(our_idxs),20*log10(abs(y(our_idxs,:)*Scale)+eps));
        %imagesc(t,f(our_idxs),(abs(y(our_idxs,:)*Scale)+eps));
        axis xy; 
        colormap(jet)
    end
    xlabel('Time')
...
x = P{1}; 
% Если есть второй аргумент, то это - длина БПФ
if (length(P) > 1) && ~isempty(P{2})
    nfft = P{2};
else
    nfft = min(length(x),256); % Если нет - минимальное из length(x) и 256
end
% Третий аргумент - частота дискретизации
...
% Пятый аргумент - степень перекрытия в отсчётах
